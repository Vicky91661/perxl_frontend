...
import 'package:pexllite/screens/taskhistory.dart';
...
class _HomeScreenState extends State<TaskHomeScreen> {
          ...
          if (task.status == 0)
            ListTile(
              ...
              trailing: Checkbox(
                onChanged: (value) {
                  setState(() {
                    // Ensure 'value' is non-null with the null-coalescing operator (??)
                    task.status = (value ?? false) ? 1 : 0; // If 'value' is null, treat it as false
                  });
                  DatabaseHelper.instance.updateTask(task);
                  Fluttertoast.showToast(
                    msg: (value ?? false) ? "Task Completed" : "Task Reassigned", // Ensure non-null here too
                ...
                value: task.status == 1, // Simplified expression for boolean check
              ),
